...
function PlantsVsZombies() {
	...
	this.drawHud = function() {
		...
		availablePlantsPreview = ["img-moneyplantPreview", "img-peashooterPreview", "img-nutPreview", "img-nutTallPreview"];

		//Available plants
		for(var i in this.availablePlants ) {
			map.drawImage( $(availablePlantsPreview[i]), (80 * i), 10 , 50 , 50);
		}
	...
	this.pauseGame = function(e, textIn) {
		if( _this.isPaused ) {
			...
			cxt.fillText(textIn, this.width/2 - cxt.measureText(text).width/2, this.height/2 + 20);
			cxt.restore();
	...
	this.gameOver = function() {
		if (zombiesCrossed >= 2) {
			  clearInterval(checkOver);
				_this.pauseGame("0", "GAME OVER | F5 to play again!");
		}
	}

	var checkOver = setInterval(this.gameOver, 3000);
	...
	if( this.zombies.length > 1 ) {
			...
			var text = 'GAME OVER | PRESS F5 to restart'
